...
import{t as e}from"./main-Cnsc14sN.js";import{S as t,a as n,b as r,h as i,i as a,o,r as s,s as c,t as l,v as u,x as d,y as f}from"./three.module-C16rtgpX.js";var p=`
...
`;function m(n,{tier:r=`full`}={}){let m=r===`full`,_;try{_=new l({canvas:n,antialias:m,alpha:!0,powerPreference:`high-performance`,failIfMajorPerformanceCaveat:!1})}catch(e){return console.warn(`[Brand-Alchemy] WebGL init failed; keeping static background.`,e),null}let v=m?2:1,y=Math.min(window.devicePixelRatio||1,v);_.setPixelRatio(y),_.setSize(window.innerWidth,window.innerHeight),_.setClearAlpha(0),_.toneMapping=4,_.toneMappingExposure=1;let b=new d;b.background=g();let x=new u(36,window.innerWidth/window.innerHeight,.1,100);x.position.set(0,0,3.6);let S=m?5e3:1e3,C=new Float32Array(S*3),w=new Float32Array(S),T=Math.PI*(3-Math.sqrt(5));for(let e=0;e<S;e++){let t=1-e/(S-1)*2,n=Math.sqrt(1-t*t),r=T*e,i=.82+h(e)*.34;C[e*3+0]=Math.cos(r)*n*i,C[e*3+1]=t*i,C[e*3+2]=Math.sin(r)*n*i,w[e]=h(e*7.13+2)}let E=new a;E.setAttribute(`position`,new s(C,3)),E.setAttribute(`aRand`,new s(w,1));let D={uTime:{value:0},uEnergy:{value:0},uProgress:{value:0},uHover:{value:0},uFlow:{value:.2},uFreq:{value:.85},uSize:{value:m?26:30},uPixelRatio:{value:y},uColorA:{value:new c(16757084)},uColorB:{value:new c(2089098)},uColorC:{value:new c(3727871)}},O=new t({uniforms:D,transparent:!0,depthWrite:!1,blending:2,vertexShader:`
    ...
    `}),k=new f(E,O);b.add(k);let A=null;m&&e(async()=>{let{EffectComposer:e,EffectPass:t,RenderPass:n,BloomEffect:r,KernelSize:i}=await import(`./build-DFAspLsd.js`);return{EffectComposer:e,EffectPass:t,RenderPass:n,BloomEffect:r,KernelSize:i}},__vite__mapDeps([0,1]),import.meta.url).then(({EffectComposer:e,EffectPass:t,RenderPass:n,BloomEffect:r,KernelSize:i})=>{let a=new e(_);a.addPass(new n(b,x)),a.addPass(new t(x,new r({intensity:.7,luminanceThreshold:.5,luminanceSmoothing:.4,mipmapBlur:!0,kernelSize:i.LARGE}))),a.setSize(window.innerWidth,window.innerHeight),A=a}).catch(e=>console.warn(`[Brand-Alchemy] bloom unavailable:`,e));function j(){let e=window.innerWidth,t=window.innerHeight,n=e/t;x.aspect=n,x.updateProjectionMatrix(),_.setSize(e,t),A&&A.setSize(e,t);let r=n>1?i.clamp((n-1)*1.2,0,1.7):0;k.position.x=r,x.position.z=n<.85?4.6:3.6}j();let M={x:0,y:0},N={x:0,y:0},P=0;function F(e){M.x=e.clientX/window.innerWidth*2-1,M.y=e.clientY/window.innerHeight*2-1,P=1}window.matchMedia(`(pointer: fine)`).matches&&window.addEventListener(`pointermove`,F,{passive:!0});let I=window.scrollY||0,L=0,R=0,z=0;function B(){return Math.max(1,(document.documentElement.scrollHeight||document.body.scrollHeight)-window.innerHeight)}function V(){let e=window.scrollY||window.pageYOffset||0;L+=Math.min(Math.abs(e-I)/38,.7),I=e,z=i.clamp(e/B(),0,1)}window.addEventListener(`scroll`,V,{passive:!0});let H=new o,U=0,W=!0;function G(){if(!W)return;U=requestAnimationFrame(G);let e=Math.min(H.getDelta(),.05);D.uTime.value+=e,R+=L,L=0,R*=.9,R=Math.min(R,1.5),D.uEnergy.value+=(R-D.uEnergy.value)*.25,D.uProgress.value+=(z-D.uProgress.value)*.06,N.x+=(M.x-N.x)*.045,N.y+=(M.y-N.y)*.045,D.uHover.value+=(P-D.uHover.value)*.05,P*=.96,k.rotation.y=D.uTime.value*.05+N.x*.4,k.rotation.x=-N.y*.3,A?A.render():_.render(b,x)}G();function K(){j()}window.addEventListener(`resize`,K,{passive:!0});function q(){document.hidden?(W=!1,cancelAnimationFrame(U)):W||(W=!0,H.getDelta(),G())}return document.addEventListener(`visibilitychange`,q),{setHover(e){P=Math.max(P,e)},destroy(){W=!1,cancelAnimationFrame(U),window.removeEventListener(`resize`,K),window.removeEventListener(`pointermove`,F),window.removeEventListener(`scroll`,V),document.removeEventListener(`visibilitychange`,q),E.dispose(),O.dispose(),b.background&&b.background.dispose&&b.background.dispose(),A&&A.dispose(),_.dispose()}}}function h(e){let t=Math.sin(e*127.1+311.7)*43758.5453;return t-Math.floor(t)}function g(){let e=document.createElement(`canvas`);e.width=512,e.height=512;let t=e.getContext(`2d`);t.fillStyle=`#0a0a0f`,t.fillRect(0,0,512,512);let i=t.createRadialGradient(370,140,0,370,140,360);i.addColorStop(0,`rgba(255,153,51,0.22)`),i.addColorStop(1,`rgba(255,153,51,0)`),t.fillStyle=i,t.fillRect(0,0,512,512),i=t.createRadialGradient(120,410,0,120,410,340),i.addColorStop(0,`rgba(31,224,138,0.15)`),i.addColorStop(1,`rgba(31,224,138,0)`),t.fillStyle=i,t.fillRect(0,0,512,512),i=t.createRadialGradient(256,270,0,256,270,300),i.addColorStop(0,`rgba(56,225,255,0.06)`),i.addColorStop(1,`rgba(56,225,255,0)`),t.fillStyle=i,t.fillRect(0,0,512,512);let a=new n(e);return a.colorSpace=r,a}export{m as mountScene};